        ...
        return
                sf.withTransaction((s, t) ->
                        unsentNotifications.map(notifications -> notifications.stream().map(n -> {
                                            ...
                                            return Mail.withText(u.getEmail(), MAIL_SUBJECT, String.format("Dear %s %s (%s), we inform you about\n%s", u.getFirstName(), u.getLastName(), u.getPreferredUserName(), n.getUserMessage()));
                                                })
                                                .collect(Collectors.toList())
                                )
                                .call(mails -> {
